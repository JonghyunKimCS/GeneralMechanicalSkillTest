...
{
	float score(float _a, float _nu, float _eta, float _sz, float _st, float _T_err)
	{
		D.set_param(_a, _nu, _eta, _sz, _st, _T_err);
		...
		*/
		...
		auto toOptimize = [](OptCalculation<float>& optCalculation)
		{
			optCalculation.result = score(optCalculation.get_parameter("a"),
										...
										optCalculation.get_parameter("T_err"));
		...
		optBoundaries.add_boundary({ 0.001f, 0.5f, "T_err" });
